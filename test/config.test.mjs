...
test('release config uses a signed updater and NSIS', async () => {
  ...
  const workspaceUi = await read('runtime/assets/balto-ui.js')
  const workspaceCapability = JSON.parse(await read('src-tauri/capabilities/workspace.json'))
  assert.doesNotMatch(app, /check_for_updates|install_update|updateButton/)
  assert.doesNotMatch(html, /id="update-button"|id="update-row"/)
  assert.match(workspaceUi, /tauriInvoke\('check_for_updates'\)/)
  assert.match(workspaceUi, /tauriInvoke\('install_update'\)/)
  assert.match(workspaceUi, /updateButton\.addEventListener\('click', \(\) => void installAvailableUpdate\(\)\)/)
  assert.match(workspaceUi, /setInterval\(\(\) => void checkForUpdates\(\), 60000\)/)
  assert.equal(workspaceCapability.local, false)
  assert.deepEqual(workspaceCapability.remote.urls, ['http://127.0.0.1:3080/*'])
  assert.deepEqual(workspaceCapability.permissions, ['core:default'])
...
test('first launch sets itself up and opens the familiar workspace', async () => {
  ...
  assert.doesNotMatch(html, /class="balto-runner"/)
  assert.doesNotMatch(html, /class="balto-track"/)
  assert.match(html, />Download Qwen /)
  assert.match(html, />Build local stack /)
  assert.match(html, /Download Qwen <small>23 GB<\/small>/)
  assert.match(html, /Build local stack <small>18 GB<\/small>/)
  assert.doesNotMatch(html, /id="setup-step-system"/)
  assert.match(html, /id="setup-step-stack"/)
  assert.doesNotMatch(html, /class="step-progress"/)
  assert.match(html, /id="setup-overall-bar"/)
  assert.doesNotMatch(html, /SGLang runtime for the RTX 5090|Docker engine|resumable Xet transfer/)
  ...
  assert.match(setup, /stage = 'parallel-assets'/)
  assert.match(setup, /function Ensure-ParallelAssets/)
  assert.match(setup, /Start-HiddenNativeProcess -FilePath 'docker' -Arguments @\('pull', \$containerImage\)/)
  assert.match(setup, /Start-ModelDownloader \$cacheVolume/)
  assert.match(setup, /HF_XET_HIGH_PERFORMANCE=1/)
  assert.match(setup, /workspaceProgress = \$workspaceProgress/)
  assert.match(app, /'parallel-assets': \['model', 'stack'\]/)
  assert.match(app, /elements\.setupSteps/)
  assert.doesNotMatch(styles, /\.step-progress/)
  assert.match(styles, /\.setup-overall-progress/)
